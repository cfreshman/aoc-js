...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    if (1) {
      const muls = ii.re(/mul\((\d{1,3}),(\d{1,3})\)/g).m(m => A(m).s(1).num)
      const rs = muls.m(([a, b]) => a * b)
      p1(sum(rs))
    }
    if (1) {
      const insts = ii.re(/do\(\)|don't\(\)|mul\((\d{1,3}),(\d{1,3})\)/g).m(m => m[1] ? ['mul', A(m).s(1).num] : [m[0]])
      let on = true
      const rs = insts.m(([type, [a, b]=[]]) => {
        if (type === `do()`) on = true
        else if (type === `don't()`) on = false
        else if (on) return a * b
        return 0
      })
      p2(sum(rs))
    }
