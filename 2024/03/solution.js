...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    if (1) {
      let muls = A(ii.MA(/mul\((\d{1,3}),(\d{1,3})\)/)).M(A).M(m => m.S(1).N())
      // l(muls)
      let rs = muls.M(([a, b]) => a * b)
      p1(sum(rs))
    }
    if (1) {
      let muls = A(ii.MA(/do\(\)|don't\(\)|mul\((\d{1,3}),(\d{1,3})\)/)).M(A).M(m => m[1] ? ['mul', m.S(1).N()] : [m[0]])
      // l(muls)
      let DO = true
      let rs = muls.M(([i, [a, b]=[]]) => {
        if (i === `do()`) DO = true
        else if (i === `don't()`) DO = false
        else if (DO) return a * b
        return 0
      })
      p2(sum(rs))
    }
