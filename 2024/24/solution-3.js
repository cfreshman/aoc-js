    ...
    if (2) {
      ...
      while (frontier.n) {
        ...
        if (node_a.value !== undefined && node_b.value !== undefined) {
          let result = undefined
          if (op === 'AND') result = { a: node_a.value, op: 'AND', b: node_b.value }
          if (op === 'OR') result = { a: node_a.value, op: 'OR', b: node_b.value }
          if (op === 'XOR') result = { a: node_a.value, op: 'XOR', b: node_b.value }
          nodes[out].value = result
          next[out].forEach(k => frontier.push(k))
        }
      }

      let zs = keys(nodes).filter(k => k.startsWith('z')).sort().reverse()
      L(zs.map(k => nodes[k].value))
