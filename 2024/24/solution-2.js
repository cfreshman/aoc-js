...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    ...
    if (2) {
      ...
      let mismatched_set = set(mismatched)
      let non_mismatched = keys(nodes).filter(k => k.startsWith('z') && !mismatched_set.has(k))
      {
        ...
        if (lls[1].length < 10) {
          ...
          for (let [a, b] of first) {
            ...
            for (let [a, b] of second) {
              let l2 = [a, b]
              let l = [l1, l2]
              let key = l.flatMap(ab => ab).sort().join(',')
              ...
              zs.sort().reverse()
              let actual = parseInt(zs.map(o => xs[o]).join(''), 2)
              L(target, actual)
              if (actual === target) {
                p2(key)
                return
