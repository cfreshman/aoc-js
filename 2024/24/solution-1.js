...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    ...
    if (1) {
      ...
      zs.sort().reverse()
      // L(zs)
      p1(parseInt(zs.map(o => xs[o]).join(''), 2))
      // p1(rs.sum)
      // p1(rs.product)
    }
    if (2) {
      ...
      let x = parseInt(keys(xs).filter(k => k.startsWith('x')).sort().reverse().map(k => xs[k]).join(''), 2)
      let y = parseInt(keys(xs).filter(k => k.startsWith('y')).sort().reverse().map(k => xs[k]).join(''), 2)
      let target = x + y
      L('target', target)
