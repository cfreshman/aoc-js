...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    let score = (A, B, P) => {
      let P1 = P[1], A1 = A[1], B1 = B[1]
      let P01 = P[0] - P[1], A01 = A[0] - A[1], B01 = B[0] - B[1]
      let ta = ((P01*B1)/B01 - P1) / ((B1*A01)/B01 - A1)
      let tb = (P1 - ta*A1) / B1
      
      let round_precision = 1e3
      ta = Math.round(ta * round_precision) / round_precision
      tb = Math.round(tb * round_precision) / round_precision
      if (ta < 0 || tb < 0 || ta % 1 !== 0 || tb % 1 !== 0) return 0
      return ta * 3 + tb
    }
    ll = ii.twoline
    if (1) {
      let rs = ll.map(lns => {
        let [a, b, prize] = lns.map(ln => ln.split(': ')[1])
        let A = a.split(', ').map(y => y.split('+')[1].num)
        let B = b.split(', ').map(y => y.split('+')[1].num)
        let P = prize.split(', ').map(y => y.split('=')[1].num)
        return score(A, B, P)
      })
      p1(rs.sum)
    }
    if (2) {
      let rs = ll.map(lns => {
        ...
        let A = a.split(', ').map(y => y.split('+')[1].num)
        let B = b.split(', ').map(y => y.split('+')[1].num)
        let P = prize.split(', ').map(y => y.split('=')[1].num + 10000000000000)
        return score(A, B, P)
      })
      p2(rs.sum)
    }
