...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    let grid = ll.grid()
    let start = grid.gvof(c => c === 'S')
    let end = grid.gvof(c => c === 'E')

    let run_cost_to = (pos) => {
      let cost_to = {}
      let explored = set()
      let frontier = [{ v:pos, cost:0 }]
      while (frontier.n) {
        let { v, cost } = frontier.shift()
        if (explored.had(v.key)) continue
        cost_to[v.key] = cost
        grid.gd4(v).map(u => {
          if (grid.gget(u) === '#') return
          frontier.push({ v:u, cost:cost+1 })
        })
      }
      return cost_to
    }
    let cost_to_start = run_cost_to(start)
    let cost_to_end = run_cost_to(end)
    let actual_cost = cost_to_end[start.key]

    let run = (offs) => {
      let cheats = set()
      let target = grid.length < 30 ? offs.length < 5 ? 64 : 76 : 100
      grid.gvfor((c, v) => {
        let start_cost = cost_to_start[v.key]
        if (start_cost === undefined) return
        let min_cost = start_cost + v.manhat(end)
        if (actual_cost - min_cost < target) return
        offs.map(off => {
          let u = v.add(off)
          let finish_cost = cost_to_end[u.key]
          if (finish_cost === undefined) return
          let cheat_cost = start_cost + u.manhat(v) + finish_cost
          if (actual_cost - cheat_cost < target) return
          cheats.add([v.key, u.key].key)
        })
      })
      return cheats.n
    }
    ...
    if (1) {
      let offs = Array.d4().map(off => ve(off).scale(2))
      p1(run(offs))
    }
    if (2) {
      let offs = []
