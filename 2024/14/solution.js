...
;(() => {
  window.solution = (ii) => U.answer(ii, (ll, p1, p2) => {
    let [W, H] = ll.n < 20 ? [11, 7] : [101, 103]
    if (1) {
      let rs = ll.map(ln => {
        let [p, v] = ln.split(' ').map(x => x.slice(2).split(',').num).map(ve)
        return { p, v }
      })
      range(100).map(i => {
        rs.map(r => {
          r.p = ve((r.p.x + r.v.x + W)%W, (r.p.y + r.v.y + H)%H)
        })
      })
      let qrs = [
        [0, 0, Math.floor(W/2), Math.floor(H/2)],
        [Math.ceil(W/2), 0, W, Math.floor(H/2)],
        [0, Math.ceil(H/2), Math.floor(W/2), H],
        [Math.ceil(W/2), Math.ceil(H/2), W, H],
      ].map(([x1, y1, x2, y2]) => {
        return rs.filter(r => r.p.x >= x1 && r.p.x < x2 && r.p.y >= y1 && r.p.y < y2).n
      })
      p1(qrs.product)
    }
    if (2) {
      ...
      ;(() => {
        for (let i = 0;; i++) {
          ...
          const grid = Array.from({ length: H }, () => Array.from({ length: W }, () => ' ')).grid()
          rs.map(r => grid[r.p.y][r.p.x] = '#')
  
          let all_explored = set()
          for (let r = 0; r < H; r++) {
            for (let c = 0; c < W; c++) {
              const v = ve(c, r)
              ...
              while (frontier.length) {
                ...
                if (explored.n >= 40) return p2(i + 1)
              }
            }
          }
        }
