...
;(() => {
  window.solution = (input) => U.answer(input, (lines, p1, p2) => {
    {
      const [T, D] = lines.map(line => line.split(':')[1].split(/ +/g).filter(x=>x).map(x => Number(x.trim())))
      const races = range(T.length).map(i => {
        const t = T[i]
        const d = D[i]
        let wins = 0
        for (let i = 0; i < t; i++) {
    ...
    {
      const [T, D] = lines.map(line => [Number(line.split(':')[1].split(/ +/g).filter(x=>x).map(x => x.trim()).join(''))])
      const races = range(T.length).map(i => {
        const t = T[i]
        const d = D[i]
        let wins = 0
  ...
  const U = {
    ...
    range: (start, stop=undefined, step=undefined) => {
